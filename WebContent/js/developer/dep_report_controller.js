(function(global, $) {
	...
	let report_location_mark = new ol.style.Style({
        image: new ol.style.Icon({
            anchor: [0.5, 1],
            src: 'images/icons/report_svg.svg',
        })
    });
...
//			global.filterwiseData(report_array);
		...
		get_layer_by_layer_id : function get_layer_by_layer_id(selected_layer){
			selected_layer_ = selected_layer;
			var form_data = {
					tableName: selected_layer_
			}
			get_layer_on_map(form_data);
		}
	}
	...
	.validate(
			{
				...
				submitHandler : function(form, e) {
					...
					try {
						...
						var table_name = $("#report_data_category").val();
...
function get_report_table_data(){
	...
	$.ajax({
        method: 'POST',
        url: window.iscdl.appData.baseURL + "citizen/query/getreportdatatable",
        data: JSON.stringify(tbl_form_data),
        contentType: 'application/json',
        async: false,
        beforeSend: function (request) {
            request.setRequestHeader('Authorization', 'Bearer '
...
function showTableData(data) {
							...
							"columnDefs" : [
									...
									},
									{
										...
										"render" : function(data, type, row, meta) {
											return '<a class="got_to" href="#" gid="' +data +'">Go-To-Location</a>';
										}
									}
									]
	...
	function goToSelectedData(g_id){
		...
		$.ajax({
            method: 'POST',
            url: window.iscdl.appData.baseURL + "citizen/query/getSelectedDataReport",
            ...
            success: function (result) {
                ...
                selected_report_vector_layer = new ol.layer.Vector({
                    source: vectorSource,
                    style: report_location_mark,
                });

                selected_report_vector_layer.getSource().on('addfeature', function () {
                    map.setExtent(vectorLayer.getSource().getExtent());
                });
                ...
                map.addLayer(selected_report_vector_layer);
	...
	$("#clear_report_form").click(function(){
		...
		$('#report_table thead').empty();
		$("#report_table").empty();
		
		
	});
	
	function get_layer_on_map(formData){
