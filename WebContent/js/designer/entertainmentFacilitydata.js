(function(global, $) {
	...
	let formid = "form_entertainment_facility";
	let geotagphotoid = "entertainment_facility_geo_tagged_photo";
	let sublayerid = "entertainment_facility_subLayerId";
	let modalid = "entertainment_facility_modal";
	let tableid = "entertainmentFacility_table";
	let wardid = "entertainment_facility_ward";
	let table, layer_id = undefined;
	let layer_name = "Entertainment Facility";
	...
	$(window).on("load", function(){
		...
		try {
			...
			$("#entertainment_facility_latitude").attr("readonly", false);
			$("#entertainment_facility_longitude").attr("readonly", false);
			window.entertainmentFacilityData.loadTable();
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					latitude : "required",
					longitude : "required",
					entertainment_facility_name : "required",
					address : "required",
					...
					entertainment_facility_name : {
						required : "Please Enter Entertainment Facility Name"
					},
					address : {
					...
					try {
						...
						var validator = $("#form_entertainment_facility").validate();
						let errorObj = {};
						...
						if(!data.entertainment_facility_name || data.entertainment_facility_name === null || data.entertainment_facility_name === ""){
							errorObj.entertainment_facility_name = "Please enter Name";
						}
			...
			table = window.entertainmentFacilityData.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.entertainmentFacilityData.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.entertainmentFacilityData.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.entertainmentFacilityData.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#entertainment_facility_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
				window.entertainmentFacilityData.loadCurrentReport(result, tableid);
			}else{
	...
	global.entertainmentFacilityData = data;
