(function(global, $) {
	...
	let formid = "form_publicToilet";
	let geotagphotoid = "public_toilet_geo_tagged_photo";
	let sublayerid = "public_toilet_subLayerId";
	let modalid = "publicToilet_modal";
	let tableid = "public_toilet_table";
	let wardid = "public_toilet_ward";
	let table, layer_id = undefined;
	let layer_name = "Public Toilets";
	...
	$(window).on("load", function(){
		...
		try {
			...
			$("#public_toilet_latitude").attr("readonly", false);
			$("#public_toilet_longitude").attr("readonly", false);
			window.publicToilet.loadTable();
	...
	let data = {
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					...
					try {
						...
						var validator = $("#form_publicToilet").validate();
						let errorObj = {};
						...
						if(!data.map_toilets_name || data.map_toilets_name === null || data.map_toilets_name === ""){
							errorObj.map_toilets_name = "Please enter Name";
						}
			...
			table = window.publicToilet.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.publicToilet.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.publicToilet.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.publicToilet.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#public_toilet_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
				window.publicToilet.loadCurrentReport(result, tableid);
			}else{
