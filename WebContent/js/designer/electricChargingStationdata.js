(function(global, $) {
	...
	let formid = "form_electric_charging_station";
	let geotagphotoid = "ecs_geo_tagged_photo";
	let sublayerid = "ecs_subLayerId";
	let modalid = "electric_charging_station_modal";
	let tableid = "electricChargingStation_table";
	let wardid = "ecs_ward";
	let table, layer_id = undefined;
	let layer_name = "Electric Charging Station";
	...
	$(window).on("load", function(){
		...
		try {
			...
			$("#ecs_latitude").attr("readonly", false);
			$("#ecs_longitude").attr("readonly", false);
			window.electricChargingStationData.loadTable();
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					latitude : "required",
					longitude : "required",
					electric_charging_station_name : "required",
					address : "required",
					...
					electric_charging_station_name : {
					...
					try {
						...
						var validator = $("#form_electric_charging_station").validate();
						let errorObj = {};
						...
						if(!data.electric_charging_station_name || data.electric_charging_station_name === null || data.electric_charging_station_name === ""){
							errorObj.electric_charging_station_name = "Please enter Name";
						}
			...
			table = window.electricChargingStationData.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.electricChargingStationData.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.electricChargingStationData.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.electricChargingStationData.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#ecs_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
				window.electricChargingStationData.loadCurrentReport(result, tableid);
			}else{
	...
	global.electricChargingStationData = data;
