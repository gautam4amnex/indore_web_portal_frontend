(function(global, $) {
	...
	let formid = "form_addVCBVMS";
	let geotagphotoid ="vcb_vms_geoTaggedPhoto";
	let sublayerid = "vcb_vms_subLayerId";
	let modalid = "addVCBVMS_modal";
	let tableid = "dep_vcb_vms_table";
	let wardid = "vcb_vms_ward";
	let table, layer_id = undefined;
	let layer_name = "VCBs/VMSs";
	...
	$(window).on("load", function(){
		...
		try {
			...
			window.depUtlityController.getLayers('add_data_category');
			 $("#vcb_vms_latitude").attr("readonly", false);
			 $("#vcb_vms_longitude").attr("readonly", false);
			 window.vmc_data.loadTable();
	...
	let data = {
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					...
					try {
						...
						var validator = $("#form_addVCBVMS").validate();
						let errorObj = {};
			...
			table = window.vmc_data.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.vmc_data.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.vmc_data.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.vmc_data.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#vcb_vms_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
			                window.vmc_data.loadCurrentReport(result, tableid);
						}else{
