(function(global, $) {
	...
	let formid = "form_addSportsFacility";
	let geotagphotoid ="sports_facility_geoTaggedPhoto";
	let sublayerid = "sports_facility_subLayerId";
	let modalid = "addSportsFacility_modal";
	let tableid = "dep_sport_facility_table";
	let wardid = "sports_facility_ward";
	let table, layer_id = undefined;
	let layer_name = "Sports Facility";
	...
	$(window).on("load", function(){
		...
		try {
				...
				window.depUtlityController.getLayers('add_data_category');
				 $("#sports_facility_latitude").attr("readonly", false);
				 $("#sports_facility_longitude").attr("readonly", false);
				 window.sports_facility_data.loadTable();
	...
	let data = {
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					...
					sports_facility_name : {
						required : "Please Enter Sports Facility Name"
					},
					address : {
					...
					try {
						...
						var validator = $("#form_addSportsFacility").validate();
						let errorObj = {};
						...
						if(!data.sports_facility_name || data.sports_facility_name === null || data.sports_facility_name === ""){
							errorObj.sports_facility_name = "Please enter sports facility Name";
						}
						if(!data.address || data.address === null || data.address === ""){
							errorObj.address = "Please enter sports facility Address";
						}
						if(!files){
			...
			table = window.sports_facility_data.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.sports_facility_data.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.sports_facility_data.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.sports_facility_data.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#sports_facility_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
                window.sports_facility_data.loadCurrentReport(result, tableid);
			}else{
	...
	global.sports_facility_data = data;
