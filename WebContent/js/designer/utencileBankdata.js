(function(global, $) {
	...
	let formid = "form_utencile_bank";
	let geotagphotoid = "utencile_bank_geo_tagged_photo";
	let sublayerid = "utencile_bank_subLayerId";
	let modalid = "utencile_bank_modal";
	let tableid = "utencile_bank_table";
	let wardid = "utencile_bank_ward";
	let table, layer_id = undefined;
	let layer_name = "Bartan/Utencile Bank";
	...
	$(window).on("load", function(){
		...
		try {
			...
			$("#utencile_bank_latitude").attr("readonly", false);
			$("#utencile_bank_longitude").attr("readonly", false);
			window.utencileBankData.loadTable();
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					...
					utencile_bank_name : {
						required : "Please Enter Utensil Bank Name"
					},
					address : {
						required : "Please Enter Utensil Bank Address"
					},
					geo_tagged_photo  : {
					...
					try {
						...
						var validator = $("#form_utencile_bank").validate();
						let errorObj = {};
						...
						if(!data.utencile_bank_name || data.utencile_bank_name === null || data.utencile_bank_name === ""){
							errorObj.utencile_bank_name = "Please enter utencile bank Name";
						}
						if(!data.address || data.address === null || data.address === ""){
							errorObj.address = "Please enter utencile bank Address";
						}
						if(!files){
			...
			table = window.utencileBankData.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.utencileBankData.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.utencileBankData.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.utencileBankData.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#utencile_bank_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
				window.utencileBankData.loadCurrentReport(result, tableid);
			}else{
