(function(global, $) {
	...
	let formid = "form_eye_do_center";
	let geotagphotoid = "eyeDonation_geo_tagged_photo";
	let sublayerid = "eyeDonation_subLayerId";
	let modalid = "eye_donation_center_modal";
	let tableid = "eyeDonation_table";
	let wardid = "eyeDonation_ward";
	let table, layer_id = undefined;
	let layer_name = "Eye Donation Center";
	...
	$(window).on("load", function(){
		...
		try {
			...
			$("#eyeDonation_latitude").attr("readonly", false);
			$("#eyeDonation_longitude").attr("readonly", false);
			window.eyeDonationCenterData.loadTable();
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					latitude : "required",
					longitude : "required",
					eye_donation_center_name : "required",
					address : "required",
					...
					eye_donation_center_name : {
						required : "Please Enter Eye Donation Center Name"
					},
					address : {
					...
					try {
						...
						var validator = $("#form_eye_do_center").validate();
						let errorObj = {};
						...
						if(!data.eye_donation_center_name || data.eye_donation_center_name === null || data.eye_donation_center_name === ""){
							errorObj.eye_donation_center_name = "Please enter eye donation center Name";
						}
			...
			table = window.eyeDonationCenterData.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.eyeDonationCenterData.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.eyeDonationCenterData.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.eyeDonationCenterData.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#eyeDonation_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
				window.eyeDonationCenterData.loadCurrentReport(result, tableid);
			}else{
	...
	global.eyeDonationCenterData = data;
