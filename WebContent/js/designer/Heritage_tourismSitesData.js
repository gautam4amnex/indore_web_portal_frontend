(function(global, $) {
	...
	let formid = "form_addtourismSites";
	let geotagphotoid = "heritage_and_tourist_geo_tagged_photo";
	let sublayerid = "heritage_and_tourist_subLayerId";
	let modalid = "heritageTourism_modal";
	let tableid = "heritage_tourism_sites_table";
	let wardid = "heritage_and_tourist_ward";
	let table, layer_id = undefined;
	let layer_name = "Heritage & Tourism Sites";
	...
	$(window).on("load", function(){
		...
		try {
			...
			$("#heritage_and_tourist_latitude").attr("readonly", false);
			$("#heritage_and_tourist_longitude").attr("readonly", false);
			window.heritageTourismSitesData.loadTable();
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					latitude : "required",
					longitude : "required",
					heritage_and_tourist_site_name : "required",
					address : "required",
					...
					heritage_and_tourist_site_name : {
						required : "Please Enter Heritage and Tourist Site Name"
					},
					address : {
						required : "Please Enter heritage and Tourist Site Address"
					},
					geo_tagged_photo  : {
					...
					try {
						...
						var validator = $("#form_addtourismSites").validate();
						let errorObj = {};
						...
						if(!data.heritage_and_tourist_site_name || data.heritage_and_tourist_site_name === null || data.heritage_and_tourist_site_name === ""){
							errorObj.heritage_and_tourist_site_name = "Please enter heritage and tourist site Name";
						}
			...
			table = window.heritageTourismSitesData.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.heritageTourismSitesData.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.heritageTourismSitesData.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.heritageTourismSitesData.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#heritage_and_tourist_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
				window.heritageTourismSitesData.loadCurrentReport(result, tableid);
			}else{
	...
	global.heritageTourismSitesData = data;
