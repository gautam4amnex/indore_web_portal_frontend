(function (global, $) {
	...
	let formid = "form_milk_booth";
	let geotagphotoid ="milk_booth_geo_tagged_photo";
	let sublayerid = "milk_booth_subLayerId";
	let modalid = "milk_booth_modal";
	let tableid = "milkBooths_table";
	let wardid = "milk_booth_ward";
	let table, layer_id = undefined;
	let layer_name = "Milk Booths";
	...
	$(window).on("load", function(){
		...
		try {
				...
				window.depUtlityController.getLayers('add_data_category');
				 $("#milk_booth_latitude").attr("readonly", false);
				 $("#milk_booth_longitude").attr("readonly", false);
				 window.milkBoothsData.loadTable();
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					...
					milk_booth_name : {
						required : "Please Enter Milk Booth Name"
					},
					address : {
						required : "Please Enter Milk Booth Address"
					},
					geo_tagged_photo  : {
					...
					try {
						...
						var validator = $("#form_milk_booth").validate();
						let errorObj = {};
						...
						if(!data.milk_booth_name || data.milk_booth_name === null || data.milk_booth_name === ""){
							errorObj.milk_booth_name = "Please enter Milk Booth Name";
						}
						if(!data.address || data.address === null || data.address === ""){
							errorObj.address = "Please enter  Milk Booth Address";
						}
						if(!files){
			...
			table = window.milkBoothsData.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.milkBoothsData.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.milkBoothsData.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.milkBoothsData.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#milk_booth_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
			                window.milkBoothsData.loadCurrentReport(result, tableid);
						}else{
