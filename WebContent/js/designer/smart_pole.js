(function(global, $) {
	...
	let formid = "form_addSmartPole";
	let geotagphotoid ="smart_pole_geoTaggedPhoto";
	let sublayerid = "smart_pole_subLayerId";
	let modalid = "addSmartPole_modal";
	let tableid = "dep_smart_pole_table";
	let wardid = "smart_pole_ward";
	let table, layer_id = undefined;
	let layer_name = "Smart pole";
	...
	$(window).on("load", function(){
		...
		try {
				...
				window.depUtlityController.getLayers('add_data_category');
				 $("#smart_pole_latitude").attr("readonly", false);
				 $("#smart_pole_longitude").attr("readonly", false);
				 window.smart_pole_data.loadTable();
	...
	let data = {
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					...
					try {
						...
						var validator = $("#form_addSmartPole").validate();
						let errorObj = {};
						...
						if(!data.smart_pole_name || data.smart_pole_name === null || data.smart_pole_name === ""){
							errorObj.smart_pole_name = "Please enter Name";
						}
			...
			table = window.smart_pole_data.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.smart_pole_data.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.smart_pole_data.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.smart_pole_data.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#smart_pole_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
                window.smart_pole_data.loadCurrentReport(result, tableid);
			}else{
