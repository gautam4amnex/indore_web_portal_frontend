(function(global, $) {
	...
	let formid = "form_hostels";
	let geotagphotoid = "hostel_geo_tagged_photo";
	let sublayerid = "hostel_subLayerId";
	let modalid = "hostels_modal";
	let tableid = "hostel_table";
	let wardid = "hostel_ward";
	let table, layer_id = undefined;
	let layer_name = "Hostels";
	...
	$(window).on("load", function(){
		...
		try {
			...
			$("#hostel_latitude").attr("readonly", false);
			$("#hostel_longitude").attr("readonly", false);
			window.hostelData.loadTable();
	...
	let data = {
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					...
					try {
						...
						var validator = $("#form_hostels").validate();
						let errorObj = {};
						...
						if(!data.hostel_name || data.hostel_name === null || data.hostel_name === ""){
							errorObj.hostel_name = "Please enter hostel Name";
						}
						if(!data.address || data.address === null || data.address === ""){
							errorObj.address = "Please enter hostel Address";
						}
						if(!files){
			...
			table = window.hostelData.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.hostelData.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.hostelData.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.hostelData.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#hostel_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
				window.hostelData.loadCurrentReport(result, tableid);
			}else{
