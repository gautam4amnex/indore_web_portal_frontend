(function(global, $) {
	...
	let formid = "form_car";
	let geotagphotoid = "car_geo_tagged_photo";
	let sublayerid = "car_subLayerId";
	let modalid = "car_and_scooter_rentals_modal";
	let tableid = "car_scooter_table";
	let wardid = "car_ward";
	let table, layer_id = undefined;
	let layer_name = "Car & Scooter Rentals";
	...
	$(window).on("load", function(){
		...
		try {
			...
			$("#car_latitude").attr("readonly", false);
			$("#car_longitude").attr("readonly", false);
			window.carScooterRentalData.loadTable();
	...
	let data = {
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					...
					try {
						...
						if(!data.name_of_rental || data.name_of_rental === null || data.name_of_rental === ""){
							errorObj.name_of_rental = "Please enter Name";
						}
			...
			table = window.carScooterRentalData.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.carScooterRentalData.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.carScooterRentalData.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.carScooterRentalData.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#car_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
				window.carScooterRentalData.loadCurrentReport(result, tableid);
			}else{
	...
	global.carScooterRentalData = data;
