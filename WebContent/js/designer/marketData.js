(function(global, $) {
	...
	let formid = "form_market";
	let geotagphotoid = "market_geo_tagged_photo";
	let sublayerid = "market_subLayerId";
	let modalid = "market_modal";
	let tableid = "market_table";
	let wardid = "market_ward";
	let table, layer_id = undefined;
	let layer_name = "Markets";
	...
	$(window).on("load", function(){
		...
		try {
			...
			window.depUtlityController.getLayers('add_data_category');
			 $("#market_latitude").attr("readonly", false);
			 $("#market_longitude").attr("readonly", false);
			 window.marketData.loadTable();
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					...
					shopping_market_name : {
						required : "Please Enter Shopping Market Name"
					},
					address : {
						required : "Please Enter Shopping Market Address"
					},
					geo_tagged_photo  : {
					...
					try {
						...
						var validator = $("#form_market").validate();
						let errorObj = {};
						...
						if(!data.shopping_market_name || data.shopping_market_name === null || data.shopping_market_name === ""){
							errorObj.shopping_market_name = "Please enter Shopping Market Name";
						}
						if(!data.address || data.address === null || data.address === ""){
							errorObj.address = "Please enter Shopping Market Address";
						}
						if(!files){
			...
			table = window.marketData.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.marketData.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.marketData.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.marketData.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#market_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
			                window.marketData.loadCurrentReport(result, tableid);
						}else{
