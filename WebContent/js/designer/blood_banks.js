(function(global, $) {
	...
	let formid = "form_addBloodBanks";
	let geotagphotoid ="blood_banks_geoTaggedPhoto";
	let sublayerid = "blood_banks_subLayerId";
	let modalid = "addBloodBanks_modal";
	let tableid = "dep_blood_bank_table";
	let table, layer_id = undefined;
	let layer_name = "Blood Banks";
	let wardid = "blood_banks_ward";
	...
	$(window).on("load", function(){
		...
		try {
			...
			window.blood_banks_data.loadTable();
		} catch (e) {
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					...
					blood_bank_name : {
						required : "Please Enter Blood Bank Name"
					},
					address : {
						required : "Please Enter Blood Bank Address"
					},
					geo_tagged_photo  : {
					...
					try {
						...
						var validator = $("#form_addBloodBanks").validate();
						let errorObj = {};
						...
						if(!data.blood_bank_name || data.blood_bank_name === null || data.blood_bank_name === ""){
							errorObj.blood_bank_name = "Please enter blood bank Name";
						}
						if(!data.address || data.address === null || data.address === ""){
							errorObj.address = "Please enter blood bank Address";
						}
						if(!files){
			...
			table = window.blood_banks_data.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.blood_banks_data.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.blood_banks_data.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.blood_banks_data.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#blood_banks_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
				window.blood_banks_data.loadCurrentReport(result, tableid);
			}else{
