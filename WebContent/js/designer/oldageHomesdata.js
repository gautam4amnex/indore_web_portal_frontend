(function(global, $) {
	...
	let formid = "form_oldage_homes";
	let geotagphotoid = "oldage_home_geo_tagged_photo";
	let sublayerid = "oldage_homes_subLayerId";
	let modalid = "oldage_homes_modal";
	let tableid = "oldageHomes_table";
	let wardid = "oldage_homes_ward";
	let table, layer_id = undefined;
	let layer_name = "Oldage Homes";
	...
	$(window).on("load", function(){
		...
		try {
			...
			$("#oldage_homes_latitude").attr("readonly", false);
			$("#oldage_homes_longitude").attr("readonly", false);
			window.oldageHomeData.loadTable();
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					...
					oldage_home_name : {
						required : "Please Enter Oldage Home Name"
					},
					address : {
						required : "Please Enter Oldage Home Address"
					},
					geo_tagged_photo  : {
					...
					try {
						...
						var validator = $("#form_oldage_homes").validate();
						let errorObj = {};
						...
						if(!data.oldage_home_name || data.oldage_home_name === null || data.oldage_home_name === ""){
							errorObj.oldage_home_name = "Please enter oldage home Name";
						}
			...
			table = window.oldageHomeData.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.oldageHomeData.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.oldageHomeData.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.oldageHomeData.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#oldage_homes_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
				window.oldageHomeData.loadCurrentReport(result, tableid);
			}else{
