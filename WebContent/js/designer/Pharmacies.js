(function(global, $) {
	...
	let formid = "form_addPharmacies";
	let geotagphotoid = "pharmacies_geoTaggedPhoto";
	let sublayerid = "pharmacies_subLayerId";
	let modalid = "addPharmasies_modal";
	let tableid = "dep_pharmacies_table";
	let wardid = "pharmacies_ward";
	let table, layer_id = undefined;
	let layer_name = "Pharmacies";
	...
	$(window).on("load", function(){
		...
		try {
			...
			$("#pharmacies_latitude").attr("readonly", false);
			$("#pharmacies_longitude").attr("readonly", false);
			window.phramaData.loadTable();
	...
	let data = {
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					...
					try {
						...
						var validator = $("#form_addPharmacies").validate();
						let errorObj = {};
						...
						if(!data.pharmacy_name || data.pharmacy_name === null || data.pharmacy_name === ""){
							errorObj.pharmacy_name = "Please enter Pharmacy Name";
						}
						if(!data.address || data.address === null || data.address === ""){
							errorObj.address = "Please enter Pharmacy Address";
						}
						if(!files){
			...
			table = window.phramaData.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.phramaData.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.phramaData.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.phramaData.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#pharmacies_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
				window.phramaData.loadCurrentReport(result, tableid);
			}else{
