(function(global, $) {
	...
	let formid = "form_orphanageHome";
	let geotagphotoid = "orphanage_home_geo_tagged_photo";
	let sublayerid = "orphanage_home_subLayerId";
	let modalid = "orphanageHome_modal";
	let tableid = "orphanage_table";
	let wardid = "orphanage_home_ward";
	let table, layer_id = undefined;
	let layer_name = "Orphanage Homes";
	...
	$(window).on("load", function(){
		...
		try {
			...
			$("#orphanage_home_latitude").attr("readonly", false);
			$("#orphanage_home_longitude").attr("readonly", false);
			window.orphanageData.loadTable();
	...
	let data = {
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					...
					try {
						...
						var validator = $("#form_orphanageHome").validate();
						let errorObj = {};
						...
						if(!data.orphanage_home_name || data.orphanage_home_name === null || data.orphanage_home_name === ""){
							errorObj.orphanage_home_name = "Please enter Name";
						}
			...
			table = window.orphanageData.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.orphanageData.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.orphanageData.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.orphanageData.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#orphanage_home_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
				window.orphanageData.loadCurrentReport(result, tableid);
			}else{
