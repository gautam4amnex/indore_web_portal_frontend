(function(global, $) {
	...
	let formid = "form_sfz";
	let geotagphotoid = "sfz_geo_tagged_photo";
	let sublayerid = "sfz_subLayerId";
	let modalid = "street_food_zones_modal";
	let tableid = "food_zone_table";
	let wardid = "sfz_ward";
	let table, layer_id = undefined;
	let layer_name = "Food zones";
	...
	$(window).on("load", function(){
		...
		try {
			...
			$("#sfz_latitude").attr("readonly", false);
			$("#sfz_longitude").attr("readonly", false);
			window.foodZoneData.loadTable();
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					latitude : "required",
					longitude : "required",
					street_food_zone_name : "required",
					address : "required",
					...
					try {
						...
						var validator = $("#form_sfz").validate();
						let errorObj = {};
						...
						if(!data.street_food_zone_name || data.street_food_zone_name === null || data.street_food_zone_name === ""){
							errorObj.street_food_zone_name = "Please enter Name";
						}
			...
			table = window.foodZoneData.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.foodZoneData.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.foodZoneData.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.foodZoneData.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#sfz_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
				window.foodZoneData.loadCurrentReport(result, tableid);
			}else{
