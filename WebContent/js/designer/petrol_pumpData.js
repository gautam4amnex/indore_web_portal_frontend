(function(global, $) {
	...
	let formid = "form_petrolPump";
	let geotagphotoid = "petrolPump_geoTaggedPhoto";
	let sublayerid = "petrolPump_subLayerId";
	let modalid = "petrolPump_modal";
	let tableid = "petrolPump_table";
	let wardid = "petrolPump_ward";
	let table, layer_id = undefined;
	let layer_name = "Petrol Pump";
	...
	$(window).on("load", function(){
		...
		try {
			...
			$("#petrolPump_latitude").attr("readonly", false);
			$("#petrolPump_longitude").attr("readonly", false);
			window.petrolPumpData.loadTable();
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					...
					petrol_pump_name : {
						required : "Please Enter Petrol Pump Name"
					},
					address : {
						required : "Please Enter Petrol Pump Address"
					},
					geo_tagged_photo  : {
					...
					try {
						...
						var validator = $("#form_petrolPump").validate();
						let errorObj = {};
						...
						if(!data.petrol_pump_name || data.petrol_pump_name === null || data.petrol_pump_name === ""){
							errorObj.petrol_pump_name = "Please enter petrol pump Name";
						}
						if(!data.address || data.address === null || data.address === ""){
							errorObj.address = "Please enter petrol pump Address";
						}
						if(!files){
			...
			table = window.petrolPumpData.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.petrolPumpData.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.petrolPumpData.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.petrolPumpData.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#petrolPump_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
				window.petrolPumpData.loadCurrentReport(result, tableid);
			}else{
