(function(global, $) {
	...
	let formid = "form_dustbin";
	let geotagphotoid = "dustbin_geo_tagged_photo";
	let sublayerid = "dustbin_subLayerId";
	let modalid = "dustbin_locations_modal";
	let tableid = "dustbin_locations_table";
	let wardid = "dustbin_ward";
	let table, layer_id = undefined;
	let layer_name = "Dustbin Locations";
	...
	$(window).on("load", function(){
		...
		try {
			...
			$("#dustbin_latitude").attr("readonly", false);
			$("#dustbin_longitude").attr("readonly", false);
			window.dustbinLocationsData.loadTable();
	...
	let data = {
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					...
					try {
						...
						var validator = $("#form_dustbin").validate();
						let errorObj = {};
			...
			table = window.dustbinLocationsData.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.dustbinLocationsData.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.dustbinLocationsData.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.dustbinLocationsData.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			layer_id = row.id;
			$('#dustbin_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
				window.dustbinLocationsData.loadCurrentReport(result, tableid);
			}else{
	...
	global.dustbinLocationsData = data;
