(function(global, $) {
	...
	let formid = "form_freeWifi";
	let geotagphotoid = "free_wifi_geo_tagged_photo";
	let sublayerid = "free_wifi_subLayerId";
	let modalid = "freeWifi_data_modal";
	let tableid = "free_wifi_table";
	let wardid = "free_wifi_ward";
	let table, layer_id = undefined;
	let layer_name = "Free WIFI Locations";
	...
	$(window).on("load", function(){
		...
		try {
			...
			window.depUtlityController.getLayers('add_data_category');
			$("#free_wifi_latitude").attr("readonly", false);
			$("#free_wifi_longitude").attr("readonly", false);
			window.freewifi.loadTable();
	...
	let data = {
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					...
					try {
						...
						var validator = $("#form_freeWifi").validate();
						let errorObj = {};
			...
			table = window.freewifi.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.freewifi.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.freewifi.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.freewifi.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#free_wifi_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
				window.freewifi.loadCurrentReport(result, tableid);
			}else{
