(function(global, $) {
	...
	let formid = "form_addWaterATN";
	let geotagphotoid = "wateratm_geoTaggedPhoto";
	let sublayerid = "wateratm_subLayerId";
	let modalid = "addWaterATM_modal";
	let tableid = "dep_water_atm_table";
	let wardid = "wateratm_ward";
	let table, layer_id = undefined;
	let layer_name = "Water ATM";
	...
	$(window).on("load", function(){
		...
		try {
			...
			window.depUtlityController.getLayers('add_data_category');
			 $("#wateratm_latitude").attr("readonly", false);
			 $("#wateratm_longitude").attr("readonly", false);
			 window.waterATMData.loadTable();
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					...
					water_atm_name : {
						required : "Please Enter Water ATM Name"
					},
					address : {
					...
					try {
						...
						var validator = $("#form_addWaterATN").validate();
						let errorObj = {};
						...
						if(!data.water_atm_name || data.water_atm_name === null || data.water_atm_name === ""){
							errorObj.water_atm_name = "Please enter Water ATM Name";
						}
			...
			table = window.waterATMData.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.waterATMData.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.waterATMData.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.waterATMData.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#wateratm_subLayerId').val(row.sub_layer_id);
			...
			if(result.data.length > 0){
                window.waterATMData.loadCurrentReport(result, tableid);
			}else{
