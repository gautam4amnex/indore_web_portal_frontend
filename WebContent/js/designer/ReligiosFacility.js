(function(global, $) {
	...
	let formid = "form_addReligiousFacility";
	let geotagphotoid = "religios_facility_geoTaggedPhoto";
	let sublayerid = "religios_facility_subLayerId";
	let modalid = "addReligiosFacility_modal";
	let tableid = "dep_religious_facility_table";
	let wardid = "religios_facility_ward";
	let table, layer_id = undefined;
	let layer_name = "Religious Facility";
	...
	$(window).on("load", function(){
		...
		try {
			...
			window.depUtlityController.getLayers('add_data_category');
			 $("#religios_facility_latitude").attr("readonly", false);
			 $("#religios_facility_longitude").attr("readonly", false);
			 window.religiosData.loadTable();
		...
		setRequiredEvent : function(){
			...
			$('form[id="' + formid + '"]').validate({
				rules : {
					latitude : "required",
					longitude : "required",
					religious_facility_name : "required",
					address : "required",
					...
					try {
						...
						var validator = $("#form_addReligiousFacility").validate();
						let errorObj = {};
						...
						if(!data.religious_facility_name || data.religious_facility_name === null || data.religious_facility_name === ""){
							errorObj.religious_facility_name = "Please enter Name";
						}
			...
			table = window.religiosData.createDatatable({
				id : id,
				...
				columnDefs : [{
					...
					"render" : function(data, type, row, meta) {
						if(row.is_approved === null) {
							return "<button name='edit' class='btn action-btn' data-row='"+JSON.stringify(row)+"' data-toggle='modal' data-target='#" + modalid + "'  onclick='window.religiosData.updatedata(event)'> " + 
							"<span class='fa fa-edit' aria-hidden='true'></span></button>"+
							"<button name='approve' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.religiosData.approveRejectData(event,true)'>"+
							"<span class='fa fa-check' aria-hidden='true'></span></button>" +
							"<button name='reject' class='btn action-btn' data-row='"+JSON.stringify(row) + "'" +
							"onclick='window.religiosData.approveRejectData(event,false)'><span class='fa fa-remove' aria-hidden='true'></span></button>";
						} else {
							return "NA";
						}
					}
			...
			let row = $(event.currentTarget).data('row');
			layer_id = row.id;
			$('#religios_facility_subLayerId').val(row.sub_layer_id);
			...
			let result = JSON.parse(response.responseText);
			 if(result.data.length > 0){
                 window.religiosData.loadCurrentReport(result, tableid);
         }else{
